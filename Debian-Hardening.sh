...
usage() {
    cat <<'USAGE'
Usage: Debian-Hardening.sh [options]

Options:
  --authorized-admins "user1,user2"   Comma-separated list of local accounts allowed in the sudo/admin group (default: root only).
...
initialize_logging() {
    ...
    LOG_FILE="$OUTPUT_DIR/debian_hardening_${timestamp}.log"
    touch "$LOG_FILE"
    chmod 600 "$LOG_FILE"
...
install_security_packages() {
    local packages=("ufw" "unattended-upgrades" "apt-listchanges" "apt-transport-https" "libpam-pwquality" "debsums" "auditd")
...
configure_unattended_upgrades() {
    local distro_codename="$(. /etc/os-release && echo "$VERSION_CODENAME")"
...
remove_insecure_packages() {
    local packages=(telnet rsh-client rsh-redone-server rsh-server talk xinetd nis tftp tftpd inetutils-inetd vsftpd ftp)
    local to_remove=()
    local pkg
    ...
    if (( ${#to_remove[@]} > 0 )); then
        ...
        log_info "No insecure network packages found to purge."
    fi
}

disable_insecure_services() {
    local services=(avahi-daemon cups nfs-server rpcbind vsftpd bluetooth smbd nmbd telnet.socket tftp.service)
    local svc
...
main() {
    ...
    run_step "Installing baseline security packages" install_security_packages
    ...
    run_step "Auditing interactive users" auditing_interactive_users
    run_step "Auditing administrator group" audit_admin_group
    run_step "Removing insecure network packages" remove_insecure_packages
    run_step "Disabling unused or insecure services" disable_insecure_services
